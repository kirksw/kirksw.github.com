...
const source = await readFile('src/lib/posts.ts', 'utf8');
test('production filtering is centralized in allPosts', () => {
  assert.match(source, /import\.meta\.env\.DEV/);
  assert.match(source, /!data\.draft/);
});
...
test('production output retains post image links and copied assets', async () => {
  ...
  assert.ok(page.includes('src="/posts/agentic-sdlc/images/agentic-sdlc-infographic.webp"'));
...
test('comments preserve URL-based legacy thread lookup', async () => {
